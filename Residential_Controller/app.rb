...
class Elevator
    def initialize(_id, _floor_amount)
        ...
        @current_floor = 0
        @previous_floor = 0
        @door = "closed"    # false = closed / true = open
    ...
    def add_stop(requestFloor, direction)
        if direction == @current_direction and direction == @name_list[0] and requestedFloor >= @current_floor
            @stop_list.push(requestFloor)

        elsif direction == @current_direction and direction == @name_list[1] and requestedFloor <= @current_floor
            @stop_list.push(requestFloor)
    ...
    def points_update(requestFloor, direction)
        ...
        if @status != @name_list[2]
            dif_last_stop = @stop_list[-1] - requestFloor
    ...
    def run()
        ...
        while @stop_list.length != 0
            if @stop_list.length !=0
                ...
                if @stop_list[0] == @current_floor
                    puts "elevator #{@ID} arrived at floor #{@stop_list[0]}"
                    @door_state
...
class Column
    ...
    def requestElevator(requestedFloor, direction)
        ...
        for i in 0..(@elevator_list.length)
            @elevator_list[i].points_update requestedFloor, direction
        end

        @elevator_list.sort_by do |elevator|
            elevator.points
        end
